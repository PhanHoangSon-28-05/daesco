...
<div>
    <div class="modal fade" id="crudSystemModal" tabindex="-1" role="dialog" aria-hidden="true" wire:ignore.self>
                    ...
                    <div class="modal-body" wire:loading.remove wire:target="modalSetup">
                        ...
                        @if ($action == 'delete')
                            <div class="container-fluid">
                                <div class="row">
                                    <span>Bạn có muốn xóa system "{{ $name }}"?</br>
                                    </span>
                                </div>
                            </div>
                        @else
                            <div class="container-fluid">
                                ...
                                <div class="row mb-2">
                                    <label class="crud-label p-0 mt-2 mb-0">Links:</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control crud-input" wire:model.lazy="links">
                                    </div>
                                    @error('links')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
...
@push('script')
    <script>
        $(document).ready(function() {
            $('#crudSystemModal').on('show.bs.modal', function(e) {
                var id = e.relatedTarget.getAttribute('data-system-id') ?? 0
                @this.call('modalSetup', id)
            })

            $(document).on('closeCrudSystem', function() {
                $('#crudSystemModal').modal('hide')
            })
